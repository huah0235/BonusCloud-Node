...
#	ln -s /lib/ld-linux-aarch64.so.1 /opt/lib/ld-linux-aarch64.so.1 >/dev/null 2>&1
	...
	ln -s /lib/ld-linux-armhf.so.3 /opt/lib/ld-linux.so.3
...
#	ln -s libjson-c.so.3.0.1 libjson-c.so.2 >/dev/null 2>&1
 ...
 #              ln -s libjson-c.so.3.0.1 libjson-c.so.2
               ...
               ln -s /lib/arm-linux-gnueabihf/libjson-c.so.3.0.1 /opt/lib/libjson-c.so.2
               ln -s /lib/arm-linux-gnueabihf/libjson-c.so.3.0.1 /opt/lib/libjson-c.so.2.0.2
...
func_bound_bcode()
{
	...
	rm -rf $BXC_SSL_RES $BXC_SSL_CA $BXC_SSL_CRT $BXC_SSL_KEY $BXC_INFO_LOC $BXC_EMAIL_LOC $BXC_BCODE_LOC
               ...
               chmod -R 777 /root/bxc
               chmod  777 /root/bxc/bxc-json.sh
